...
export async function POST(request) {
  ...
  if (!scriptUrl || !formSecret) {
    return Response.json({ error: 'Audit form is not configured.' }, { status: 500 });
  }
  ...
  payload.set('sourceButton', incoming.get('sourceButton') || 'audit');
  ...
  try {
    ...
    return Response.json({ error: 'Unable to submit audit form.' }, { status: 502 });
  }
}
